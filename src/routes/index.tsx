...
function DashboardPage() {
  ...
  return <>
    ...
    <div className="space-y-6 p-4 sm:p-6 xl:p-8">
      ...
      <SectionCard title="Maharashtra District-wise Student Engagement" subtitle="Representative district visualization based on demo engagement values"><MaharashtraMap districts={districts} selectedId={selectedDistrict} onSelect={selectDistrict} /></SectionCard>
...
function AttentionList({ title, items }: { title: string; items: { name: string; value: string }[] }) { return <div className="rounded-md border border-border p-5"><h3 className="text-sm font-bold">{title}</h3><div className="mt-4 divide-y divide-border">{items.map((item, i) => <div key={item.name} className="flex items-center justify-between py-3"><div className="flex items-center gap-3"><span className="grid size-6 place-items-center rounded-sm bg-destructive-soft text-[10px] font-bold text-destructive">{i+1}</span><span className="text-xs font-medium">{item.name}</span></div><span className="text-xs font-bold text-destructive">{item.value}</span></div>)}</div></div>; }
